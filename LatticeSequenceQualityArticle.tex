...
\end{equation}
where $\{\phi(\cdot)\}_{i=0}^\infty$ is the van der Corput sequence in base $b$.  (The base is suppressed in our notation fo simplicty.) In this case $\vh$ must be a generalized integer as defined in \cite[Section 2]{HicNie03a}. An example of a two-dimensional shifted extensible lattice is shown in \Cref{fig:lattice-plot}. 
...
\end{equation}
Also note that the set of $b^m$ points starting at $\lambda b^m$ is a shift by $\phi(\lambda \times b^m)$ of the first $b^m$ points:
...
\begin{multline} \label{eq:Niedbd}
    P_{\alpha}(\vh,\vgamma,b^m,\onetos) \le C_{P}(b,\alpha,\vgamma,\epsilon,s)
...
\begin{theorem} \label{thm:one}
  ...
  \max(1, (\log n)^{\alpha(s+1)})
    ...
    \begin{equation} \label{eq:mainresultoneb}
        P_\alpha(\vh,\vgamma,n,\onetos) 
        \le \frac {\widehat{C}_{P}(b,\alpha,\vgamma,\epsilon,s)}{n} \qquad
      \forall n , s \in \naturals.
    \end{equation}

    \end{enumerate}

    \item However, if $n = \lambda b^p$, where $\lambda$ is a fixed integer, 
    
    \begin{enumerate}
        \renewcommand{\labelenumii}{\roman{enumii}.}

    \item However, if $n = \lambda b^p$, where $\lambda$ is a fixed integer, then $P_\alpha$, has an upper bound that is similar to the case of $n=b^m$.
...
\Cref{thm:one} makes no assumptions on the coordinate weights. In this section, we demonstrate how to eliminate the logarithmic terms in our upper bounds in Theorem \Cref{thm:one}---which grow exponentially as $s \to \infty$---by assuming a summability condition of the coordinate weights. In fact, we eliminate the $s$-dependence completely.

\begin{theorem} \label{thm:two} \textbf{new version}
Suppose $\alpha > 1$.
\begin{enumerate}\renewcommand{\labelenumi}{\alph{enumi}.}
    \item If $\sum_{j = 1}^{\infty} \gamma_j ^{\alpha/(1+2\delta)} < \infty$ for some $\delta$ satisfying $0 < \delta < (\alpha -1)/2$, then
\begin{equation*}
     P_\alpha(\vh,\vgamma,n,\onetos)
        \leq \frac{\widetilde{C}_P(\alpha, \vgamma, \delta)}{n^{1-\delta}} \frac{b^{\delta}(b - 1) }{b^{\delta} - 1} \qquad
        \forall s = 1,2,\ldots.
\end{equation*}
...
We showed in \Cref{thm:one} that the best decay we can get for arbitrary $n$ is nearly like $\Order(n^{-1 + \delta})$, so the weakest assumption on the coordinate weights we can make to achieve this decay is by taking $a = \alpha$.  Substituting this bound into \eqref{eq:Palphan} we get: %with $a = \alpha$ we get: %and following the same argument as in \eqref{eq:mainresultone}, we get the following upper bound on $P_{\alpha}$: 

\begin{align*}
    P_\alpha(\vh, \vgamma, n, \onetos) 
    &\leq \frac{(b - 1)}{n} \widetilde{C}_P(\alpha, \vgamma, \delta)  \Bigg\{ 
        b^{m(1-\alpha/\alpha + \delta)} + b^{(m-1)(1-\alpha/\alpha + \delta)} + \cdots \\
        & \qquad \qquad 
     + b^{0(1-\alpha/\alpha + \delta)}
    \Bigg\} \\
    &= \frac{(b - 1)}{n} \widetilde{C}_P(\alpha, \vgamma, \delta) \Bigg\{\frac{b^{(m+1)\delta} -1 }{b^{\delta} - 1} \Bigg\} \\
    &\leq \frac{\widetilde{C}_P(\alpha, \vgamma, \delta)}{n^{1-\delta}} \frac{b^{\delta}(b - 1) }{b^{\delta} - 1}.
\end{align*}

For the case $n = \lambda b^p$, we proved in \Cref{thm:one} we proved that the fastest decay we can achieve is $\Order(n^{-\alpha})$. The weakest assumption we can make on the coordinate weights in this case to achieve this decay is  $a = 1$. Then, the upper bound on $P_\alpha$ for $n = \lambda b^p$ becomes: 

\begin{align*}
        P_\alpha(\vh, \vgamma, \lambda b^p, \onetos) 
    &\leq \frac{(b - 1)}{\lambda b^p} \widetilde{C}_P(\alpha, \vgamma, \delta)  \Bigg\{ 
        b^{m(1-\alpha + \delta)} + b^{(m-1)(1-\alpha + \delta)} + \cdots \\
        & \qquad \qquad 
     + b^{p(1-\alpha + \delta)} \Bigg\} \\
    &= \frac{(b - 1)}{\lambda b^p} \widetilde{C}_P(\alpha,  \vgamma, \delta) \Bigg\{ \frac{b^{p( 1 - \alpha + \delta)}(1 - b^{(m-p+1)(1-\alpha +\delta)})}{1 - b^{1 - \alpha + \delta}} \Bigg\} \\
    &= \frac{(b - 1)}{\lambda b^{p\alpha - \delta}} \widetilde{C}_P(\alpha, \vgamma, \delta) \Bigg\{ \frac{1 - b^{(m-p+1)(1-\alpha+\delta)}}{1 - b^{1 - \alpha + \delta}} \Bigg\} \\
    &\leq \frac{\lambda^{\alpha - 1 - \delta }\widetilde{C}_P(\alpha, \vgamma, \delta)}{n^{\alpha - \delta}} \frac{(b-1)}{1 - b^{1 - \alpha + \delta}}. 
\end{align*}
...
Note that for lattices, i.e., $n=b^m$, and for equal weights $\vw = b^{-m} \vone $, it follows that $P_{\alpha,2}^2(\vh,\vgamma,b^m,\onetos, b^{-m} \vone) = P_{2\alpha}(\vh,\vgamma,b^m,\onetos)$ by definition.


For positive integer $\alpha$, we may again appeal to Bernoulli polynomials as we did in \eqref{eq:P2alpha}:
\begin{align*}
\nonumber
P_{\alpha,2}^2(\vh,\vgamma,n,\onetos, \vw)
...
\begin{theorem} \label{thm:three}% not corollary? 
...
Corollary 1 in \cite{HicEtal10a} states that if the worst case error bound for a Hilbert space like the one we have defined, and for the preferred sample size, $n = 1, b, b^2, \ldots$ is $\Order (b^{-m \alpha'})$ and $\alpha' > 1$, then for arbitrary sample size it is $\Order \bigl (n^{-1} \log(n) \bigr)$.  

$P_{\alpha,2}^2(\vh,\vgamma,b^m,\onetos) \le C b^{-m \alpha'}$ for $m =0, 1, 2, \ldots$, then $P_{\alpha,2}^2(\vh,\vgamma,n,\onetos) \le C n^{-\alpha'}$ for $n =2, 3, \ldots$.
Let S = (xk)k#0 be a lattice sequence in base b for which the worst
case error in a shift-invariant reproducing kernel Hilbert space H(K) satisf ies
e (Sb( , K) + C
,
b(-−! for all ( = 0, 1, . . . ,
for some constants C, ! > 0. Then for every N # 2, the quasi-Monte Carlo rule
using the f irst N points of the lattice sequence has the worst case error satisfying
e(SN, K) + C
logb N
Nmin(!,1)
.
