...
\begin{abstract} Extensible lattices are a common choice of 
nodes for approximating multidimensional integrals by a sample mean, typically using sample sizes of the form $n = b^m$ for a prime base $b$ and non-negative integer $m$. However, a computational time budget or early termination due to hardware failure may prevent us from choosing the preferred number of samples. In this paper, we establish an upper bound on the figure of merit $P_\alpha$ for extensible lattice sequences \emph{with arbitrary $n$}, derived as a worst-case error in a Banach space setting. 
We show that although the upper bound on $P_\alpha$ can decay no faster than $\Order(n^{-1})$ for general $n$, restricting $n$ to the form $n =\lambda b^p$, where $\lambda$ is a fixed integer, we can recover the optimal decay of nearly $\Order(n^{-\alpha})$ as $p \to \infty$. We also investigate a related figure of merit, $P_{\alpha,2}$, arising as a worst-case error for Hilbert spaces of integrands. We obtain a theoretical upper bound on $P_{\alpha,2}$ in terms of $P_{\alpha}$ for equal sample weights and show numerically that optimal sample weights can improve the cubature error bound even further.
...
Since this construction depends on $n$, a new point set must be generated each time the sample size changes. To address this, lattices were later defined extensibly, i.e., not for a fixed $n$, allowing the lattice to be extended without discarding previously generated points. Extensible lattice sequences, proposed in \cite{Mai81a} and \cite{HicEtal00}, take the form 
\begin{equation} \label{eq:extlat}
    \{\vz_i = \vh\phi(i)+ \vDelta \pmod{\vone} : i=0,1, \ldots \} \in [0,1)^s,
\end{equation}
where $\{\phi(\cdot)\}_{i=0}^\infty$ is the van der Corput sequence in base $b$.  (The base is suppressed in our notation for simplicity.) In this case $\vh$ must be a generalized integer as defined in \cite[Section 2]{HicNie03a}. An example of a two-dimensional shifted extensible lattice is shown in \Cref{fig:lattice-plot}. 
...
\end{equation}
Also note that the set of $b^m$ points starting at $\lambda b^m$ for positive integer $\lambda$ is a shift by $\phi(\lambda \times b^m)$ of the first $b^m$ points:
...
This notation follows \cite[(4)]{HicNie03a} but we 
%replace 1:s in \cite.. by s, 2 bullets
\begin{itemize}
    \item Do not assume the first $n$ points form a lattice and
    \item Replace their $\onetos$ by $s$.
\end{itemize}
For an extensible lattice with $n = b^m$, the quality measure $P_\alpha(\vh,\vgamma,n, s)$ takes on a simple form.  Note that
...
\begin{equation} \label{eq:Palphadual}
    P_\alpha(\vh,\vgamma,b^m,s) = \sum_{\vk \in B(\vh,m,1:s)} \tr(\vk,\vgamma)^{-\alpha},
\end{equation}
where $B(\vh,m,s) : = \{\vk \in  \integers^s \setminus \{\vzero\} : \vk^T \vh \pmod{b^m} = 0\}$ is the \emph{dual lattice} minus the origin.  This corresponds to \cite[(3)]{HicNie03a}.  
...
\begin{equation} \label{eq:P2alpha}
P_{2\alpha}(\vh,\vgamma,b^m,s) = -1 + \frac{1}{n}\sum_{i = 0}^{n-1}  
...
\begin{multline} \label{eq:Niedbd}
    P_{\alpha}(\vh,\vgamma,b^m,s) \le C_{P}(b,\alpha,\vgamma,\epsilon,s)
...
\begin{align*}
    \MoveEqLeft{P_{\alpha}(\vh,\vgamma,1,s)} \\
...
\begin{multline} \label{eq:Palphaextm}
    P_{\alpha}(\vh,\vgamma,b^m,s) \\
    \le C_{P}(b,\alpha,\vgamma,\epsilon,s)
...
\section{The figure of merit $P_{\alpha}(\vh,\vgamma,n,s)$ for  $n$ other than $b^m$} \label{sec:Palphabd}
Starting from the upper bound for $P_{\alpha}(\vh,\vgamma,n,s)$ for $n$ a non-negative power of the base, $b$, it is possible to obtain an analogous result arbitrary $n$: 

\begin{theorem} \label{thm:one}
    For fixed integer base $b \ge 2$, a fixed smoothness parameter $\alpha > 1$, a fixed vector of coordinate weights $\vgamma \in [0,\infty)^\infty$, and a fixed $\epsilon > 0$, 
    
    \begin{enumerate}
    \renewcommand{\labelenumi}{\alph{enumi}.}
        \item For arbitrary $n \in \naturals$,
        
        \begin{enumerate}
        \renewcommand{\labelenumii}{\roman{enumii}.}
            \item There exists a generating vector for the lattice, $\vh$, for which the figure of merit, $P_\alpha$, has the following upper bound for arbitrary positive integer $n$:
    \begin{multline} \label{eq:mainresultone}
        P_\alpha(\vh,\vgamma,n,s) 
        \le \frac {C_{P}(b,\alpha,\vgamma,\epsilon,s)}{n} \frac{(b-1)}{1 - b^{1-\alpha}} \\
        \times 
  \max(1, (\log n)^{\alpha(s+1)})
      [\max(1,\log \log (n+1))]^{\alpha(1+\epsilon)} \\
      \forall n , s \in \naturals.
    \end{multline}
    This is the same  constant, $C_{P}(b,\alpha,\vgamma,\epsilon,s)$, and the same power of logarithm as in \eqref{eq:Palphaextm} for $n = b^{m}$, but the power of $n$ is fixed at $-1$.

    \item The powers of log may be eliminated in favor of a larger constant, as follows:
    \begin{equation} \label{eq:mainresultoneb}
        P_\alpha(\vh,\vgamma,n,s) 
        \le \frac {C_{P}(b,\alpha,\vgamma,\epsilon,s) S(b, \alpha, \epsilon, s)}{n} \qquad
      \forall n , s \in \naturals,
    \end{equation}
    where the constant $S(b, \alpha, \epsilon, s)$ is defined in \eqref{eq:infseries}.

    \end{enumerate}

    \item However, if $n = \lambda b^p$ for some positive integer $p$ and some positive integer  $\lambda$ that is relatively prime with respect to $b$, then,%
%    \begin{enumerate}
%        \renewcommand{\labelenumii}{\roman{enumii}.}
%   \item 
$P_\alpha$, has an upper bound that is similar to the case of $n=b^m$:
    \begin{multline} \label{eq:mainresulttwo}
         P_\alpha(\vh,\vgamma,n,s)
         \le \frac{\lambda^{\alpha -1}{C}_{P}(b,\alpha, \vgamma,\epsilon,s)}{n^{\alpha}}  \frac {(b-1)} {1 - b^{1- \alpha}}  \\
         \times \max(1, (\log n )^{\alpha(s+1)})) [\max(1,\log \log (
    n+1))]^{\alpha(1+\epsilon)} \\
    \forall b \in \natzero, \ n, \lambda, s \in \naturals.
    \end{multline} 
    Again, the constant $C_{P}(b,\alpha,\vgamma,\epsilon,s)$ and the powers of the logarithms are the same as in \eqref{eq:Palphaextm}. Now, the power of $n$ is similar to that in \eqref{eq:Palphaextm}, but there is an extra factor of $\lambda^{\alpha -1}(b-1)/(1 - b^{1-\alpha})$.  Thus it is especially useful for small $\lambda$. (The parameter $p = \log_b(n/\lambda)$ does not appear above because it can be written in terms of $n$ and $\lambda$.)

%    \item Moreover, the powers of log may be eliminated in favor of a larger constant, as follows:
%    \begin{equation} \label{eq:mainresulttwob}
%       P_\alpha(\vh,\vgamma,\lambda b^p,s)
%         \le \frac{\lambda^{\alpha -1}{C}_{P}(b,\alpha, \vgamma,\epsilon,s) S(b, \alpha, \epsilon, s)}{n^{\alpha-\delta}} \qquad
%      \forall n , s \in \naturals.
%    \end{equation}
...
Before giving the proof of this theorem, we note that 
...
The figure of merit, $P_\alpha(\vh,\vgamma,n,s)$, depends on how small these quantities can be made by a good choice of $\vh$, namely,
\begin{align} \label{eq:Palphan}
      \nonumber
      P_\alpha(\vh,\vgamma,n,s) & = \sum_{\vk \in \integers^s \setminus\{\vzero\}} \abs{\appxint_n(\me^{2 \pi \sqrt{-1} \vk^T \cdot})}\tr(\vk,\vgamma)^{-\alpha} \qquad \text{by \eqref{eq:wcerrPalpha}} \\
      \nonumber
      & \le \frac {1}{n} \sum_{\ell = 0}^{m} b^{m-\ell} n_{m - \ell} \sum_{\vk \in B(\vh,m-\ell,s)} \tr(\vk,\vgamma)^{-\alpha}
      \qquad \text{by \eqref{eq:ihatarbn}}\\
      & = \frac {1}{n} \sum_{\ell = 0}^{m} b^{m-\ell} n_{m - \ell} P_\alpha(\vh,\vgamma,b^{m-\ell},s)
...
\begin{align} \label{eq:sameargument}
      ...
      P_\alpha(\vh,\vgamma,n,s) \\ & 
      \nonumber 
       \le  \frac {1}{n} \left\{ b^m n_m P_\alpha(\vh,\vgamma,b^m,s) + b^{m-1} n_{m-1} P_\alpha(\vh,\vgamma,b^{m-1},s) \right . \\
       \nonumber
       & \left . \qquad + \cdots + b^0 n_0 P_\alpha(\vh,\vgamma,b^0,s)  \right\} \\
       \nonumber
       &\le \frac {(b-1)}{n} \left\{ b^m  P_\alpha(\vh,\vgamma,b^m,s) + b^{m-1}  P_\alpha(\vh,\vgamma,b^{m-1},s) \right . \\
       \nonumber
       & \qquad \left . + \cdots + b^0  P_\alpha(\vh,\vgamma,b^0,s)  \right\} 
      \qquad \text{since $n_m \le b-1$}\\
      \nonumber
      & \le \frac {(b-1)C_{P}(b,\alpha,\vgamma,\epsilon,s)}{n} \\
      \nonumber
...
\begin{align*}
\MoveEqLeft P_\alpha(\vh,\vgamma,n,s) \\
      & \le \frac {(b-1)C_{P}(b,\alpha,\vgamma,\epsilon,s)}{n} \frac{(1 - b^{(m+1)(1-\alpha)})}{1 - b^{1-\alpha}}
...
Unfortunately, the upper bound decays only like $\Order(n^{-1+\delta})$.  This is due to the $m = 0$ term in the sum above.  This suggests our later result in \eqref{eq:mainresulttwo} that if the small powers of $b$ vanish, we can get a bound with a better power of $n$.

\subsection{Proof of \eqref{eq:mainresultoneb}}  We may remove the logarithmic factors in favor of a larger constant.  Since $\alpha > 1$, the infinite series 
\begin{multline} \label{eq:infseries}
    S(b, \alpha, \epsilon, s) : = b^{0}\max(1, (\log b^{0})^{\alpha(s+1)}) [\max(1,\log \log (
    b^{0}+1))]^{\alpha(1+\epsilon)} \\
    + b^{1(1-\alpha)}\max(1, (\log b^{1})^{\alpha(s+1)}) [\max(1,\log \log (
    b^{1}+1))]^{\alpha(1+\epsilon)}
    + \cdots \\
    + b^{m(1-\alpha)}\max(1, (\log b^{m})^{\alpha(s+1)}) [\max(1,\log \log (
    b^m+1))]^{\alpha(1+\epsilon)} + \cdots 
\end{multline}
converges.  This implies \eqref{eq:mainresultoneb} by \eqref{eq:sameargument}.
...
\begin{align*}
    ...
    & \le \frac {1}{\lambda b^p} \sum_{\ell = 0}^{m-p} b^{m-\ell} n_{m -\ell} P_\alpha(\vh,\vgamma,b^{m-\ell},s) \qquad \text{by \eqref{eq:Palphadual}}
\end{align*}
...
\begin{align} \label{eq:sameargument1}
    ...
    \MoveEqLeft[1]
   P_\alpha(\vh,\vgamma,\lambda b^p,s)  \\
    &\le \frac {1}{\lambda b^p} \left\{ b^m n_m P_\alpha(\vh,\vgamma,b^m,s) + b^{m-1} n_{m-1} P_\alpha(\vh,\vgamma,b^{m-1},s) \right. \\
     \nonumber
    &\qquad + \cdots + b^p n_p P_\alpha(\vh,\vgamma,b^p,s) 
    \bigg\} \\ 
     \nonumber
    & \le \frac {(b-1)}{\lambda b^p}  
    \left\{
    b^m  P_\alpha(\vh,\vgamma,b^m,s) + b^{m-1}  P_\alpha(\vh,\vgamma,b^{m-1},s) \right. \\
     \nonumber
    &\qquad + \cdots + b^p  P_\alpha(\vh,\vgamma,b^p,s) 
    \bigg\} \\
     \nonumber
    & \le \frac {(b-1)}{\lambda b^p} C_{P}(b,\alpha,\vgamma,\epsilon,s) \\
     \nonumber
    &\qquad \times \left\{ b^{m(1-\alpha)}\max\left(1, (\log b^m)^{\alpha(s+1)}\right) \max\left(1,\log \log (b^m+1)\right)^{\alpha(1+\epsilon)} \right.\\ 
     \nonumber
    & \qquad  + b^{(m-1)(1-\alpha)}\max\left(1, (\log b^{(m-1)})^{\alpha(s+1)}\right) \\
    & \nonumber \qquad \qquad \times \max\left(1,\log \log (b^{(m-1)}+1)\right)^{\alpha(1+\epsilon)}\\
     \nonumber
    &  \qquad + \cdots \\
...
\begin{align*}
     P_\alpha(\vh,\vgamma,\lambda b^p,s)
     & \le \frac { (b-1) C_{P}(b,\alpha, \vgamma, \epsilon,s)}{\lambda b^p} \left\{ b^{p(1-\alpha)} \cdot \frac{(1 - b^{(m-p+1)(1- \alpha )})}{1 - b^{1- \alpha}}\right\} \\
    & \qquad  \times \max(1, (\log b^{m})^{\alpha(s+1)})) [\max(1,\log \log (
    b^m+1))]^{\alpha(1+\epsilon)} \\
    &\leq  \frac {(b-1)C_{P}(b,\alpha, \vgamma, \epsilon,s)}{\lambda b^{p\alpha}}  \frac{1 - b^{(m-p+1)(1- \alpha )}}{1 - b^{1- \alpha}} \\
    &\qquad \times \max(1, (\log b^{m})^{\alpha(s+1)})) [\max(1,\log \log (
    b^m+1))]^{\alpha(1+\epsilon)} \\
    &\le \frac{\lambda^{\alpha -1}{C}_{P}(b,\alpha, \vgamma,\epsilon,s)}{n^{\alpha}}  \frac {(b-1)} {1 - b^{1- \alpha}} \\
...
\subsection{The figure of merit $P_{\alpha}(\vh,\vgamma,\lowercase{n},s)$ when the coordinate weights satisfy summability condition }

\Cref{thm:one} makes no assumptions on the coordinate weights. In this section, we demonstrate how to eliminate the logarithmic terms in our upper bounds in \Cref{thm:one}---which grow exponentially as $s \to \infty$---by assuming a summability condition of the coordinate weights. In fact, we eliminate the $s$-dependence completely.

\begin{theorem} \label{thm:two}
Suppose $\alpha > 1$.
\begin{enumerate}\renewcommand{\labelenumi}{\alph{enumi}.}
    \item If $\sum_{j = 1}^{\infty} \gamma_j ^{\alpha/(1+2\delta)} < \infty$ for some $\delta$ satisfying $0 < \delta < (\alpha -1)/2$, then
\begin{equation} \label{eq:thmtwoa}
     P_\alpha(\vh,\vgamma,n,s)
        \leq \frac{\widetilde{C}_P(b,\alpha, \alpha/(1 + 2\delta), \vgamma, \delta)}{n} \frac{b - 1 }{1 - b^{-\delta}} \qquad
        \forall n, s \in \naturals.
\end{equation}

\item If $\sum_{j = 1}^{\infty} \gamma_j < \infty$, then for any fixed $\delta $ such that $0 < \delta < \alpha - 1$ and any fixed $\lambda \in \naturals$:
\begin{equation*}
     P_\alpha(\vh,\vgamma,\lambda b^p,s)
        \leq \frac{\lambda^{\alpha - 1 - \delta }\widetilde{C}_P(b,\alpha, \vgamma, \delta)}{n^{\alpha - \delta}}\frac{(b-1)}{1 - b^{1 - \alpha + \delta}} \qquad 
        \forall \lambda, n, s \in \naturals.
\end{equation*}
\end{enumerate}
...
\begin{equation} \label{eq:nosbd}
      P_\alpha(\vh,\vgamma,b^m,s)
        \leq \widetilde{C}_P(\alpha, a, \vgamma, \delta) (b^m)^{-\alpha/a + \delta},
\end{equation}
for $a \in [1, \alpha]$, provided that  $\sum_{j = 1}^{\infty} \gamma_j^a < \infty$. There are two cases.

The first case is to recover the $\Order(n^{-1})$ decay as in \eqref{eq:mainresultoneb} but without the dimension dependence.  Choose a small positive $\delta$ that is less than $(\alpha -1)/2$, and let $a = \alpha/(1 + 2\delta)$.   
Substituting \eqref{eq:nosbd} into \eqref{eq:Palphan} we get: %with $a = \alpha$ we get: %and following the same argument as in \eqref{eq:mainresultone}, we get the following upper bound on $P_{\alpha}$: 

\begin{align*}
    \MoveEqLeft{P_\alpha(\vh, \vgamma, n, s)} \\
    &\leq \frac{(b - 1)}{n} \widetilde{C}_P(b,\alpha, \alpha/(1 + 2\delta), \vgamma, \delta)  \Bigl[
        b^{m(1-\alpha/a + \delta)} + b^{(m-1)(1-\alpha/a + \delta)} + \cdots \\
        & \qquad \qquad 
     + b^{0(1-\alpha/a+ \delta)} \Bigr] \\
    &\leq \frac{(b - 1)}{n} \widetilde{C}_P(b,\alpha, \alpha/(1 + 2\delta), \vgamma, \delta)  \Bigg[ 
        b^{-m \delta} + b^{-(m-1)\delta} + \cdots + 1 \Big] \\
    &\leq \frac{\widetilde{C}_P(b,\alpha, \alpha/(1 + 2\delta), \vgamma, \delta)}{n} \frac{b - 1 }{1 - b^{-\delta}}.
\end{align*}

For the case $n = \lambda b^p$, we proved in \Cref{thm:one} we proved that the fastest decay we can achieve is nearly $\Order(n^{-\alpha})$. The weakest assumption we can make on the coordinate weights in this case to achieve this decay is  $a = 1$. Then, from \eqref{eq:nosbd}, the upper bound on $P_\alpha$ for $n = \lambda b^p$ becomes: 

\begin{align*}
        P_\alpha(\vh, \vgamma, \lambda b^p, s) 
    &\leq \frac{(b - 1)}{\lambda b^p} \widetilde{C}_P(b,\alpha, 1, \vgamma, \delta)  \Bigg\{ 
        b^{m(1-\alpha + \delta)} + b^{(m-1)(1-\alpha + \delta)} + \cdots \\
        & \qquad \qquad 
     + b^{p(1-\alpha + \delta)} \Bigg\} \\
    &= \frac{(b - 1)}{\lambda b^p} \widetilde{C}_P(b,\alpha, 1, 1, \vgamma, \delta) \Bigg\{ \frac{b^{p( 1 - \alpha + \delta)}(1 - b^{(m-p+1)(1-\alpha +\delta)})}{1 - b^{1 - \alpha + \delta}} \Bigg\} \\
    &= \frac{(b - 1)}{\lambda b^{p\alpha - \delta}} \widetilde{C}_P(b,\alpha, 1, \vgamma, \delta) \Bigg\{ \frac{1 - b^{(m-p+1)(1-\alpha+\delta)}}{1 - b^{1 - \alpha + \delta}} \Bigg\} \\
    &\leq \frac{\lambda^{\alpha - 1 - \delta }\widetilde{C}_P(b,\alpha, 1, \vgamma, \delta)}{n^{\alpha - \delta}} \frac{(b-1)}{1 - b^{1 - \alpha + \delta}}. 
\end{align*}
...
In \Cref{fig:p_alpha_O} we plot the upper bound on $P_2(\vh,[0.1,0.05],n,1\!\!:\!\!2)$ for all positive integers $n \leq 2^{10}$. The decay is nearly $\Order(n^{-2})$ when $n$ is a power of $2$, and closer to $\Order(n^{-1})$ for arbitrary $n$, as expected. For $n =\lambda 2^p$ with $\lambda = 1, 3, 5, 7$, \Cref{fig:/p_alpha_lambda_O} shows that the upper bound %on $P_2(\vh,[0.1, 0.05],\lambda 2^p,1:2)$  
decays nearly like $\Order(n^{-2})$ in all cases, as proven in \Cref{thm:one}, with generally smaller values for smaller $\lambda$.
...
\]
 \citep{Kei96}.
 We approximate its integral using the sample mean over the first  $n = 1, 2, \ldots, 2^{15}$ points of a shifted lattice. In \Cref{fig:keister_n} we plot absolute errors for all positive integers $n \leq 2^{15}$, while \Cref{fig:keister_lambda} focuses  on $n = \lambda 2^p$ with the same values of $\lambda$ as in \Cref{fig:/p_alpha_lambda_O}.
...
P_{\alpha,2}(\vh,\vgamma,n,s) : = \sup_{\norm[F_{\alpha,2}]{f} \le 1 } \abs{\int_{[0,1)^{s}} f(\vx) \, \dif \vx - \appxint_n(f)} \\
\le \sup_{\norm[F_{\alpha}]{f} \le 1 } \abs{\int_{[0,1)^{s}} f(\vx) \, \dif \vx - \appxint_n(f)} = P_{\alpha}(\vh,\vgamma,n,s)
\end{multline}

\subsection{Derivation of exact formulae involving arbitrary sample weights}
...
\begin{align*}
\nonumber
\MoveEqLeft{\sup_{\norm[\cf_{\alpha,2}]{f} \le 1} \abs{\int_{[0,1)^{s}} f(\vx) \, \dif \vx - \appxint_n(f)}^2} \\
& = \sup_{\norm[\cf_{\alpha,2}]{f} \le 1} \sum_{\vk \in \integers^s \setminus\{\vzero\}} \abs{\appxint_n(\me^{2 \pi \sqrt{-1} \vk^T \cdot})}^2\tr(\vk,\vgamma)^{-2\alpha} \\
&=\sum_{\vk \in \integers^s \setminus\{\vzero\}} \sum_{i,j = 0}^{n-1} \me^{2 \pi \sqrt{-1} (\vx_i - \vx_j)} w_i w_j \tr(\vk,\vgamma)^{-2\alpha} \\
&= \underbrace{ -1 + \sum_{i,j = 0}^{n-1} w_i w_j \sum_{\vk \in \integers^s} \me^{2 \pi \sqrt{-1} (\vx_i - \vx_j)} \tr(\vk,\vgamma)^{-2\alpha}}_{P_{\alpha,2}^2(\vh,\vgamma,n,1:s, \vw)},
\end{align*}
where $\vw = \bigl( w_i \bigr)_{i=0}^{n-1}$. 


Note that for lattices, i.e., $n=b^m$, and for equal weights $\vw = b^{-m} \vone $, it follows that $P_{\alpha,2}^2(\vh,\vgamma,b^m,s, b^{-m} \vone) = P_{2\alpha}(\vh,\vgamma,b^m,s)$ by definition.


For positive integer $\alpha$, we may again appeal to Bernoulli polynomials as we did in \eqref{eq:P2alpha}:
\begin{align*}
\nonumber
P_{\alpha,2}^2(\vh,\vgamma,n,s, \vw)
& = -1 + \sum_{i,j = 0}^{n-1} w_i w_j  
\prod_{\ell = 1}^s [1 + \tilde{\gamma}_{\ell}^{2\alpha}B_{2\alpha}(x_{i,\ell} - x_{j, \ell}\bmod 1)] \\
& = - 1 + \vw^T \mK \vw, \\
\text{where } \mK  & = \biggl( \prod_{\ell = 1}^s [1 + \tilde{\gamma}_{\ell}^{2 \alpha}B_{2\alpha}(x_{i,\ell} - x_{j, \ell} \bmod 1 )]\biggr)_{i,j=0}^{n-1} ,
\end{align*}
and $\tilde{\gamma}_{\ell}$ was defined in \eqref{eq:gammatilde}.
...
where the constraint ensures that constant functions are integrated exactly. This problem has a known solution but we derive it for the sake of completeness. We solve it using the method of Lagrange multipliers. 
...
\[
\vw_{\opt} := \frac{\mK^{-1}\vone}{\vone^T \mK^{-1}\vone }.
\]

The cost of solving for the weights $\vw$ is $\Order(sn^2)$ to evaluate the kernel and $\Order(n^3)$ to invert the Gram matrix, $\mK$.  (We assume that evaluating one element of the Gram matrix is proportional to the dimension.)  However, since $\mK$ is Toeplitz, we can reduce the cost of solving for optimal sample weights to $\Order(sn+n^2)$.   %$\Order(sn) + \Order(n^2)$.

\subsection{Error bounds in the Hilbert space setting}

\begin{theorem} \label{thm:three}
The upper bounds derived in \Cref{thm:one} and \Cref{thm:two} also hold for $P_{\alpha,2}$.
\end{theorem}

\begin{proof}
    This follows immediately by the inequality in \eqref{Pa2Parelation}.
\end{proof}

Corollary 1 in \cite{HicEtal10a} states that if the worst case error bound for a Hilbert space like those we consider here is $Cb^{-m \alpha})$ for $\alpha \ge 1$ for the preferred sample size, $n = 1, b, b^2, \ldots$, then for arbitrary sample size it is $C n^{-1} \log(n)$. In  \Cref{thm:three} our hypothesis is not on the error bound but on the parameter $\alpha$ defining the Hilbert space.  The corresponding result in \eqref{eq:thmtwoa} avoids the extra $\log(n)$ factor for $\alpha > 1$, granted by inflating the leading constant.
...
Figures \Cref{fig:ratio-floor} and \ref{fig:ratio-ceil} compare $P_{1,2}$ at each positive integer $n \leq 2^7$ to its value at the nearest power-of-two sample sizes. %that are powers of 2. 
...
Future work could explore the performance of lattices with arbitrary $n$ in concrete numerical test cases, such as option pricing or integration problems described in \cite[Integration]{simulationlib} and establish a theoretical upper bound on $P_{\alpha,2}$ (done).
