...
\begin{align}\label{eq:Ralphadual}
    R_\alpha(\vh,n) %&= \sum_{\vk \in \tilde Z_n \setminus\{\vzero\}} \abs{\me^{2 \pi \sqrt{-1} \vk^T \vDelta} \bbone_{B(\vh,m)}(\vk)}\tr(\vk)^{-\alpha} \\
    &=  \sum_{\vk \in \tilde B(\vh,m)} \tr(\vk)^{-\alpha} 
\end{align} % add citation
...
\begin{align*}
    ...
    b^{1}+1)]^{\alpha(1+\epsilon)} \right\}\\
      & \qquad \qquad \qquad \alpha \ge 1 \qquad \text{by \eqref{eq:Palphaextm}}\\ %note to self: edit
