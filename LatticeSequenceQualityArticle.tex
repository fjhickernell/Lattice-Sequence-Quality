...
\begin{align*}
      ...
      & \le \frac {1}{n} \cdot C_{R}(\alpha,\epsilon,s) \frac{(1 - 2^{(m+1)(1-\alpha)})}{1 - 2^{1-\alpha}}% add C_R and the worst log
\end{align*}

With \[P_\alpha(\vh,2^0) = -1 + \left[1 + 2 \zeta(\alpha) \right]^d
      \le C_{R}(\alpha,\epsilon,s) 2^0 \max \{1, \log(2^{0})\} \max \{1,[\log \log (2^0+1)]^{\alpha(1+\epsilon)} \} % 2^0
